...
// ==============================
"""
Access control describes who and how Types, Properties, Variables, and Methods (etc.) can be accessed.
The main options are:

public - everyone can read and write to this property (DEFAULT)
internal - only your swift code (within framework/app) can read and write the property
fileprivate - only swift code within the same file can read and write the property
private - most restrictive option. Property is only available insed methods belonging to the type or its extensions.
private(set) - A version of private that allows for public reading, but private setting
"""
...
// POLYMORPHISM
"""
This is a fundamental aspect of Object Oriented Programming (OOP) and class inheritance.
If classes that inherit share the same basic behaviour, but have different ways they may do the same behaviour
(for example a Dog and a Cat are both animals that make sounds, but the actual sounds are different), they can be said
to be polymorphs of each other. Or more directly, polymorphs of a unified super class (e.g. Animal). Here is a code example:
"""
class AnimalPoly {
    var name: String
...
let mysteryAnimal = AnimalPoly(name: "?")
let gin = DogPoly(name: "Fido", breed: "Labradoodle")
let tonic = CatPoly(name: "Tonic", colour:"Grey")

mysteryAnimal.makeSound()
gin.makeSound()
tonic.makeSound()

// The magic of polymorphism really comes in when storing each of these as a collection
var householdZoo: [AnimalPoly] = [mysteryAnimal, gin, tonic]
for pet in householdZoo {
    pet.makeSound()
}
"""
As you can see from above, even though
mysteryAnimal,   gin,    and   tonic    have the types
<AnimalPoly>, <DogPoly>, and <CatPoly>, respectively, they can all be collected as their superclass of AnimalPoly.
Thus we can treat them as the same type in a loop like above, but acess each objects concrete methods specific to their actual Type!!
"""

// TYPECASTING
"""
Typecasting is the process of treating one type as another if we know it is really that type.
For example, the above loop treated each element of the householdZoo array as an AnimalPoly type,
when in fact some of them were really different types (e.g. DogPoly, CatPoly). If we needed to actually treat
them as their specific type we can attempt to caste them as their type by:
"""
for pet in householdZoo {
    switch pet {
    case let pet as CatPoly: print("\(pet.name) is of type CatPoly a polymorph of AnimalPoly")
    case let pet as DogPoly: print("\(pet.name) is of type DogPoly a polymorph of AnimalPoly")
    default: print("\(pet.name) is the superclass type AnimalPoly")
    }
    
    // Also use optional casting in if statement for safety
    // if let p as? CatPoly { ...do something... }
    
    // Or forced casting (careful, if not possible this will crash code!)
    // let p = pet as! CatPoly
}

// Can also downcast (optional - use NCO, forced - use as! [Sequence Type]) an entire loop
for _ in householdZoo as? [CatPoly] ?? [CatPoly]() {
    // This will not do anything as householdZoo contains more than just CatPoly types...
}
...
// ==============================
"""
Closures pop up everywhere in swift and iOS app development. They basically can be thought of as variables that can hold code.
This is different to a function because a closure is its OWN TYPE. Thus it can be passed in and out of a function with each.
If you are familiar with python, it is kind of like a lambda function.

let vw = UIView()
UIView.animate(withDuration: 0.5, animations: <#T##() -> Void#>) // animations is the closure part!

Closures also CAPTURE the environment when they are created in. That is, they take a copy of variables created inside them.
This can lead to memory cycles where a closure is keeping something in existence when it is otherwise not needed. e.g.

// This (not great) example would keep itself in existence
class objectA {
    property = {newInstance = objectA() } // a closure of some kind referencing the object
}


Closures are often used in methods and if they are the final argument, they can trail the function.
let vw = UIView()
...
Alternatively, if they are the only input can fo straight to the closure part
someFuncWithOnlyClosureAsAnArgument { (parameters) -> (return type) in
    (statements)
}


There is a lot more to closures than this, but the rest makes sense in practice rather than just syntactically
"""
